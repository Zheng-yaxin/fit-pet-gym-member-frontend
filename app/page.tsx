...
import { HomeMotionDirector } from "@/components/home/home-motion-director";
...
const HOME_REQUEST_TIMEOUT_MS = 2800;

function fallbackAfter<T>(fallback: T, timeoutMs = HOME_REQUEST_TIMEOUT_MS) {
...
async function settle<T>(request: Promise<T>, fallback: T) {
  try {
    return await Promise.race([request, fallbackAfter(fallback)]);
  } catch (err) {
