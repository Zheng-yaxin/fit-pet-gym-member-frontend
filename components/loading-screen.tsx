...
const steps = Array.from({ length: 5 }, (_, index) => index);
      ...
      <div className="loading-scene" aria-hidden="true">
        ...
        <div className="loading-steps">
          {steps.map((step) => (
            <motion.span
              key={step}
              animate={reduceMotion ? undefined : { opacity: [0.1, 0.62, 0.1], scale: [0.85, 1, 0.85] }}
              transition={{ duration: 0.9, repeat: Infinity, delay: step * 0.13, ease: "easeInOut" }}
            />
          ))}
        </div>
      </div>
